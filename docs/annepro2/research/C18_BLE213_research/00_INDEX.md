...
修改范围：KEY 兼容实现以官方 BLE 2.13 原二进制为基线；另提供可复现的
`AnnePro 2C` 等长名称变体，不覆盖官方样本

> 2026-07-26 范围修正：AP2D 已取消 C18 的独立 LED MCU，其 GPIO/RGB、
> HID LED Output callback 和 suspend 灯控实现不适用于 C18，不能直接移植。
> C18 原有 LED MCU 与板级灯控路径保持不变；两代固件共同确认的
> `20/07 00/01` Caps 逻辑状态仍属于 backport 契约，并由 QMK 标准 LED
> 接口桥接到 C18 路径。
>
> 同日增加的 `HEXCORE AnnePro 2C` 兼容名称镜像只等长替换两个设备名，
> 作为独立生成产物管理。协议和 KEY backport 结论仍以未修改的官方 2.13
> 镜像为准。
...
6. AP2D 的 macOS CapsLock 修复包含其 KEY 侧 1/2 B LED Output 解析和直驱灯控；本项目不移植这套硬件实现，只采用共同确认的 `20/07 00/01` Caps UART ABI，并桥接 C18 LED MCU。
7. AP2D 3.08 的 suspend 灯控依赖 KEY MCU 直驱 RGB；本项目不移植该实现。
...
10. 官方 AP2D BLE 2.13 已通过修复版 IAP 工具写入 C18，并完成启动、广播、macOS 连接和普通输入。匹配的 status-zero 回复证明传输事务成功，但 IAP 无已验证 readback，不能解释为 flash 逐字节校验；工厂信息页和 RF 校准区仍不得全片擦除。
...
| LED/RGB | C18 独立 LED MCU 原实现 | AP2D KEY MCU 直驱 | 驱动不共用；Caps 逻辑状态经 QMK 标准 LED 接口进入 C18 路径 |
...
| 4 | 四槽配对/切换状态机 | 必须 |
| 5 | Caps 状态桥接与 C18 外置 LED MCU 回归 | 必须；排除 AP2D 直驱实现 |
| 6 | 单固件运行时自动识别 BLE 版本 | 后续；等待确认版本查询命令 |
| 7 | BLE 2.13 Vendor 配置桥接 | 后续；业务 opcode 确认前关闭 |
| 8 | 量化连接参数、功耗和切换延迟 | 实机验收阶段 |

首版完成定义：同一源码内置两个 profile，可分别以 `C18_BLE205` 与
`C18_BLE213` 为默认值构建；普通键盘、媒体键、配对、四槽、串口容错和
Caps 锁定灯、USB 救援通过测试。C18 现有 LED MCU driver 保持不变，不新增
AP2D LED 兼容代码。官方 BLE 2.13 二进制保持原样；2C 名称变体由独立脚本
生成。
