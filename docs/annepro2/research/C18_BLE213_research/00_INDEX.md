...
> 2026-07-26 范围修正：AP2D 已取消 C18 的独立 LED MCU，其 HID LED
> Output、锁定灯、RGB 和 suspend 灯控实现不适用于 C18。本报告中这些内容
> 只保留为代际差异证据，不构成 backport 要求；C18 原有 LED MCU 与板级灯控
> 路径保持不变。
>
...
6. AP2D 的 macOS CapsLock 修复属于其 KEY 侧 LED Output 路径；本项目不移植该实现。
...
| LED/RGB | C18 独立 LED MCU 原实现 | AP2D KEY MCU 直驱 | 不共用；C18 路径不改 |
...
| 4 | 四槽配对/切换状态机 | 必须 |
| 5 | AP2D LED/RGB 与 suspend 灯控 | 排除；不适用于 C18 |
...
USB 救援通过测试。C18 现有 LED MCU 行为只做回归检查，不新增 AP2D LED
兼容代码。官方 BLE 2.13 二进制保持原样；2C 名称变体由独立脚本生成。
