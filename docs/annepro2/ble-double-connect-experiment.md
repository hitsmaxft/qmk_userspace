...
后续实现已经用 `c764328d2e`（`Serialize Anne Pro 2 BLE slot operations`）
替代该实验：pending 期间只保留最后一次 slot 意图，1 秒有界窗口后启动新事务；
`6a4b681982` 又把 `20/0b` 改为每次动作只发送一次。由于 `20/0c` 本身不含
slot/transaction ID，极晚旧握手的归属问题仍列为实机验证边界，而不是假装已由
延时彻底解决。
