...
2. 收到 BLE 主动发起的 `0x20/0x0c` 握手请求时，按官方主控行为回发
   ...
   driver。`0x40/0x01` 和 `0x40/0x04` ACK 只记录、不切换。
...
stateDiagram-v2
    [*] --> USB
    USB --> Pending: broadcast/connect 命令
    Pending --> Pending: 40/01 或 40/04 ACK
    Pending --> BLE: 收到 20/0c 并回发握手响应
    Pending --> USB: KC_AP2_USB 或 unpair
    BLE --> USB: KC_AP2_USB 或 unpair
    BLE --> BLE: 重复 20/0c，重复回复但不重复切换
```

`Pending` 不是 timer。等待期间按键仍走原 host driver；如果用户选择 USB 或
unpair，后到的 `20/0c` 仍会获得协议回复，但不会切换 host driver。重复握手
在 route 层是幂等的，不会反复 `clear_keyboard()`。从一个 BLE slot 切换到
另一个 slot 时，旧 BLE route 会先撤销，避免 pending 期间继续向旧链接送键。
...
- `annepro2.c`：启动和扫描时非阻塞排空 BLE UART；
- `annepro2_ble.c`：变长 parser、`20/0c` 握手回复、event gate、本地 route
  状态和可选日志；
- `annepro2_ble.h`：公开逐字节 RX 入口。
