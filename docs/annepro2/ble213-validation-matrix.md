...
BLE 2.13 功能验收已经完成。首版双 profile 仍需：

1. 换回官方 BLE 2.05，重复普通键盘、媒体、Caps、配对、四槽和断电恢复；
   日志中不得出现任何 2.13 slot 前导。
2. 若需要发布级可追溯记录，再用当前精确日志固件保存 build revision；这不
   改变已经通过的 2.13 功能结论。
3. 若运行时出现非零 ACK、迟到 ACK、半帧超时或 parser 重同步，再保存对应
   样本用于长期回归。

完整双 profile 首版验收现在只缺 BLE 2.05 实机非回归；IAP readback 与精确
revision 归档是工具链/可追溯性增强，不影响 BLE 2.13 功能 PASS。
