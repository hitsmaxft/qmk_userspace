...
### BLE 2.13 的 slot 选择前导序列

AP2D KEY 3.08 的按键事件处理函数 `0x80F0` 对目标 slot 执行：

1. 发送 `c0/17` 查询；
2. 等待约 5 ms，并读取由 `0xB114` 更新的当前 slot；
3. 若当前值不同，发送 `40/17 S` 与 `02/01 01`；
4. 等待约 20 ms，发送 `02/01 02`；
5. 再等待约 20 ms；
6. 长按路径发送 `20/0b S,1` 与 `40/01 S`，短按路径发送
   `20/24 S,2` 与 `40/04 S`。

此前只恢复了第 6 步，遗漏前五步。实机表现是 slot 1 可完成加密，而 slot
2–4 能建立物理/GATT 连接却在 SMP 阶段立即断开；这与 BLE 内部活动槽和目标
...
| `7b 12 35 00 03 00 00 7d c0 17 S` | AP2D 当前 slot 查询回复；`S=0..3` |
